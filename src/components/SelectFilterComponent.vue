<template>
    <div class="dropdown dropdown-start w-full">
        ...
        <ul tabindex="0" class="dropdown-content menu bg-base-100 rounded-box z-1 w-52 mt-1 max-h-[300px] flex-nowrap overflow-y-auto p-2 shadow-xl border border-base-300">
            <li @click="select_value(value)" v-for="value in props.values" :key="value.id" class="max-w-full overflow-hidden rounded-field">
...
// Fonction pour initialiser la sélection par défaut
const initializeDefaultSelection = () => {
  if (!props.values || props.values.length === 0) return;
  
  let defaultValue = null;
  
  // Priorité 1: Si defaultSelect est fourni, l'utiliser
  if (props.defaultSelect) {
    defaultValue = props.values.find(item => item.id === props.defaultSelect.id) || props.defaultSelect;
  }
  
  // Priorité 2: Sinon, prendre le premier élément
  if (!defaultValue) {
    defaultValue = props.values[0];
  }
  
  // Ne définir que si aucune sélection n'existe encore
  if (!select.value.id && defaultValue) {
    select.value = defaultValue;
    emit('selectValue', defaultValue);
  }
}
...
// Watcher pour surveiller les changements de defaultSelect
watch(() => props.defaultSelect, (newDefault) => {
  if (newDefault && props.values && props.values.length > 0) {
    const foundItem = props.values.find(item => item.id === newDefault.id);
    if (foundItem) {
      select.value = foundItem;
      emit('selectValue', foundItem);
    }
  }
}, { immediate: true });
